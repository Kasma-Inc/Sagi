...
class TestHiRAGSetLanguage:
    ...
    async def setup(self):
        ...
        self.set_language_tool = next(
            (tool for tool in self.hirag_tools if tool.name == "hi_set_language"), 
            None
        )
        
        logger.info(f"Available HiRAG tools: {[tool.name for tool in self.hirag_tools]}")
    ...
    async def test_set_language_tool_available(self):
        ...
        assert "hi_set_language" in tool_names, f"hi_set_language tool not found. Available tools: {tool_names}"
        assert self.set_language_tool is not None, "Set language tool should not be None"
    ...
    async def test_set_language_functionality(self):
        ...
        if not self.set_language_tool:
            logger.warning("⚠️ Skipping language functionality test - tool not available")
            return False
        ...
        for language in languages_to_test:
            ...
            try:
                ...
                assert result is not None, f"Result should not be None for language: {language}"
    ...
    async def test_invalid_language_handling(self):
        ...
        for invalid_lang in invalid_languages:
            ...
            try:
                ...
                logger.info(f"Invalid language '{invalid_lang}' raised exception (expected): {e}")
    ...
    async def test_search_functionality_both_languages(self):
        ...
        if not self.set_language_tool:
            logger.warning("⚠️ Skipping search functionality test - set_language tool not available")
            return False
        ...
        search_tool = next(
            (tool for tool in self.hirag_tools if tool.name == "hi_search"), 
            None
        )
        
        if not search_tool:
            logger.warning("⚠️ Skipping search functionality test - hi_search tool not available")
            return False
