...
def is_pdf_url(url: str) -> bool:
    ...
    for pattern in pdf_hosts_patterns:
        if pattern in hostname_lower and ('pdf' in url.lower() or 'paper' in url.lower()):
            return True
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            try:
                async with session.head(url) as response:
                    ...
                    if content_type and 'application/pdf' not in content_type and 'pdf' not in content_type:
                        logger.warning(f"URL {url} may not be a PDF (Content-Type: {content_type}), but trying anyway")
                    
                    if content_length and int(content_length) > 100 * 1024 * 1024:
                        logger.warning(f"PDF at {url} is too large ({content_length} bytes), skipping")
                        return None
            ...
            async with session.get(url) as response:
                ...
                if len(pdf_content) > 100 * 1024 * 1024:
                    logger.warning(f"Downloaded PDF is too large ({len(pdf_content)} bytes), skipping extraction")
                    return None
        ...
        try:
            ...
            for page_num in range(len(pdf_document)):
                if pages_processed >= max_pages:
                    logger.warning(f"PDF {url} has {len(pdf_document)} pages but only processed {max_pages} pages due to limit")
                    break
...
"""
            ...
            logger.info(f"Successfully extracted text from PDF {url} ({pages_processed} pages, {len(full_text)} chars)")
            return result
            ...
            try:
                ...
                for page_num, page in enumerate(pdf_reader.pages):
                    if pages_processed >= max_pages:
                        logger.warning(f"PDF {url} has {len(pdf_reader.pages)} pages but only processed {max_pages} pages due to limit (pypdf fallback)")
                        break
...
"""
                ...
                logger.info(f"Successfully extracted text from PDF {url} using pypdf fallback ({pages_processed} pages)")
                return result
                
            except Exception as fallback_error:
                logger.error(f"Both PyMuPDF and pypdf extraction failed: {fallback_error}")
                return f"""
[PDF DOCUMENT PROCESSED]
URL: {url}
Size: {len(pdf_content)} bytes
...
def enhance_search_results_with_pdf_info(search_results: str) -> str:
    ...
    if pdf_urls_found:
        ...
        pdf_summary += f"Found {len(pdf_urls_found)} PDF document(s) in search results:\n"
        ...
        pdf_summary += "Consider extracting their content for more comprehensive analysis.\n"
