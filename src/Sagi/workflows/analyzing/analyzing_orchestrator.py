    ...
    async def _get_initial_plan(
        ...
        model_response = {
            "steps": [
                {
                    "name": "Query Database",
                    "description": f"Here is the task:{{{task}}}. If the task requires obtaining data from the database, use the pg_query tool via the MCP server to retrieve the first five entries from the database. Otherwise report a warning. No analysis is needed at this step.",
                },
                {
