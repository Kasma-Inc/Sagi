...
class AnalyzingOrchestrator(BaseGroupChatManager):
    ...
    async def _get_appended_plan(
        ...
        if self._language == "en":
            ...
            analyze_prompt = get_appended_plan_prompt_cn(
                current_task=task,
                contexts_history=formatted_history,
                team_composition=self._team_description,
            )
    ...
    async def _get_prompt_templates(
        ...
        if self._language == "en":
            ...
            prompt_dict = {
                "plan_prompt": "你是一个数据库专家。使用可用的工具查询PostgreSQL数据库，并返回简洁、正确的结果。 {task}",
                "analyze_prompt": "数据现在已经被查询过了。根据数据集中的指标：\n-对于“Risk_Control”，生成风险监控建议，重点是识别异常、趋势或潜在风险。\n-对于“credit_examination”，生成信用评估建议，分析借款模式、信誉和财务行为。",
            }
        return prompt_dict
